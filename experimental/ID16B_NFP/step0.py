...
def _key(template, scan_id):
    ...
    if "${n}" not in template and "{n}" not in template:
        raise ValueError(f"Key template lacks ${{n}} or {{n}}: {template}")
    return template.replace("${n}", str(int(scan_id))).replace("{n}", str(int(scan_id)))
...
def _args(path):
    ...
    c = parser["DEFAULT"]
    a = SimpleNamespace(
        dark_file=_clean(c.get("dark_file")), dark_key=_clean(c.get("dark_key")),
        dark_nframes=c.getint("dark_nframes", fallback=51),
        flat_file=_clean(c.get("flat_file")), flat_key=_clean(c.get("flat_key")),
        flat_scan_ids=_parse_ids(c.get("flat_scan_ids"), "flat_scan_ids"),
        sample_file=_clean(c.get("sample_file")), sample_key=_clean(c.get("sample_key")),
        sample_scan_ids=_parse_ids(c.get("sample_scan_ids"), "sample_scan_ids"),
        frame_ids_spec=_clean(c.get("frame_ids", fallback="all")),
        motor_x_key=_clean(c.get("motor_x_key")), motor_y_key=_clean(c.get("motor_y_key")),
        h5_out=_clean(c.get("h5_out")), path_out=_clean(c.get("path_out", fallback="")) or None,
        ...
        niter=c.getint("niter", fallback=10), nchunk=c.getint("nchunk", fallback=4),
        vis_step=c.getint("vis_step", fallback=1), err_step=c.getint("err_step", fallback=1),
    ...
    a.rho = [float(v.strip()) for v in c.get("rho", fallback="1,2,0.00001").split(",")]
    if len(a.rho) != 3:
    ...
    if a.frame_ids_spec.lower() in ("", "all", "*"):
        ...
        missing = a.selected_scan_ids[~np.isin(a.selected_scan_ids, a.sample_scan_ids)]
        if len(missing):
...
def _dark_mean(a, shape):
    with h5py.File(a.dark_file, "r") as file:
        ...
        if a.dark_nframes > ds.shape[0]:
            raise ValueError(f"dark_nframes={a.dark_nframes} > available {ds.shape[0]}")
        out = np.asarray(ds[:a.dark_nframes], dtype="float64").mean(0).astype("float32")
    if out.shape != shape:
...
def _metadata(a, shape):
    ...
    with h5py.File(a.sample_file, "r") as file:
        for i, scan_id in enumerate(a.selected_scan_ids):
            image_key = _key(a.sample_key, scan_id)
            ds = _dataset(file, image_key)
            valid = ds.shape == shape or (ds.ndim == 3 and ds.shape == (1, *shape))
            if not valid:
                raise ValueError(f"{image_key} has shape {ds.shape}, expected {shape} or {(1,*shape)}")
            x[i] = _scalar(file, _key(a.motor_x_key, scan_id))
            y[i] = _scalar(file, _key(a.motor_y_key, scan_id))
    return x, y


def _unit_scale(unit, voxelsize):
    scales = {"m": 1, "mm": 1e-3, "um": 1e-6, "µm": 1e-6,
              "nm": 1e-9, "px": voxelsize, "pixel": voxelsize}
...
def _global_mean(a, local_ids, dark, flat_dark, shape, comm):
    ...
    with h5py.File(a.sample_file, "r") as file:
        for scan_id in local_ids:
            data = _prepare(_image(file, _key(a.sample_key, scan_id), shape),
                            dark, flat_dark, a.flat_correct)
            total += float(data.sum(dtype="float64")); count += data.size
...
def _load_data(out, a, local_ids, dark, flat_dark, mean, shape):
    with h5py.File(a.sample_file, "r") as file:
        for i, scan_id in enumerate(local_ids):
            data = _prepare(_image(file, _key(a.sample_key, scan_id), shape),
                            dark, flat_dark, a.flat_correct)
            out[i] = np.sqrt(np.abs(data / max(mean, 1e-12))).astype("float32")
...
def _bbox(path, pos, scan_ids, n, nobj, grid):
    ...
    for row in rows:
        for col in cols:
            selected.append(int(np.argmin((pos[:,0]-row)**2 + (pos[:,1]-col)**2)))
    ...
    for order, idx in enumerate(selected):
        ...
        boxes.append((x0, y0, n, n)); color = plt.cm.viridis(order/max(len(selected)-1,1))
        ax.add_patch(Rectangle((x0,y0), n,n, fill=False, color=color))
        ax.text(x0+n/2, y0+n/2, str(int(scan_ids[idx])), fontsize=6, ha="center", color=color)
    ax.add_patch(Rectangle((0,0), nobj,nobj, fill=False, color="black", lw=2))
    ax.set(xlim=(0,nobj), ylim=(nobj,0), aspect="equal",
           xlabel="global object column", ylabel="global object row",
           title=f"ID16B NFP bbox: detector={n}x{n}, selected scans={len(scan_ids)}")
    ax.grid(alpha=.25); os.makedirs(os.path.dirname(path) or ".", exist_ok=True)
    fig.savefig(path, dpi=200, bbox_inches="tight"); plt.close(fig)
    return {"local_indices": selected, "scan_ids": scan_ids[selected],
            "positions": pos[selected], "xywh": np.asarray(boxes, dtype="float32")}
...
def main():
    ...
    a = _args(sys.argv[1]); set_log_level(a.log_level)
    ...
    shape = (a.n, a.n); ntheta = len(a.selected_scan_ids)
    if rank == 0:
        dark = _dark_mean(a, shape); flat = _flat_mean(a, shape); motor_x, motor_y = _metadata(a, shape)
    else:
        dark = np.empty(shape, "float32"); flat = np.empty(shape, "float32")
        motor_x = np.empty(ntheta, "float64"); motor_y = np.empty(ntheta, "float64")
    comm.Bcast(dark, root=0); comm.Bcast(flat, root=0)
    comm.Bcast(motor_x, root=0); comm.Bcast(motor_y, root=0)

    flat_dark = flat-dark; eps = max(float(np.median(flat_dark))*1e-6, 1e-6)
    flat_dark = np.where(flat_dark > eps, flat_dark, eps).astype("float32")
    mag = a.focustodetectordistance/a.z1; voxelsize = a.detector_pixelsize/mag
    x, y = motor_x.copy(), motor_y.copy()
    if a.center_positions:
        x -= x.mean(); y -= y.mean()
    scale = _unit_scale(a.position_unit, voxelsize)
    pos = np.stack([a.pos_row_sign*y*scale/voxelsize,
                    a.pos_col_sign*x*scale/voxelsize], axis=1).astype("float32")
    ...
    if rank == 0:
        ...
        logger.info(f"sample scans            = {ntheta}/{len(a.sample_scan_ids)}: {_fmt_ids(a.selected_scan_ids)}")
        ...
        logger.info(f"position row            = [{pos[:,0].min():.3f}, {pos[:,0].max():.3f}] px")
        logger.info(f"position col            = [{pos[:,1].min():.3f}, {pos[:,1].max():.3f}] px")
        logger.info(f"nobj                    = {nobj}")

    bbox_data = None; bbox_path = os.path.join(os.path.dirname(a.h5_out) or ".", "position_bbox_sanity.png")
    if rank == 0 and a.write_position_bbox_plot:
        bbox_data = _bbox(bbox_path, pos, a.selected_scan_ids, a.n, nobj, a.position_bbox_grid_size)

    preview = None; preview_ids = a.selected_scan_ids[:min(a.preview_count, ntheta)]
    if rank == 0 and len(preview_ids):
        preview = np.empty((len(preview_ids), *shape), "float32")
        with h5py.File(a.sample_file, "r") as file:
            for i, scan_id in enumerate(preview_ids):
                preview[i] = _prepare(_image(file, _key(a.sample_key, scan_id), shape), dark, flat_dark, a.flat_correct)
    ...
    if rank == 0:
        ...
        with h5py.File(a.h5_out, "w") as out:
            for name, value in (("dark_mean",dark),("flat_mean",flat),("flat_minus_dark",flat_dark),
                                ("flat_scan_ids",a.flat_scan_ids),("sample_scan_ids",a.sample_scan_ids),
                                ("selected_scan_ids",a.selected_scan_ids),("frame_ids",a.selected_scan_ids),
                                ("motor_x",motor_x),("motor_y",motor_y),("pos",pos)):
                _write(out, name, value)
            if a.write_corrected_preview and preview is not None:
                _write(out, "corrected_preview", preview); _write(out, "preview_scan_ids", preview_ids)
            if bbox_data:
                for key, value in bbox_data.items(): _write(out, f"bbox_preview_{key}", value)
            out.attrs.update(energy_keV=a.energy, z1_m=a.z1,
                             focus_to_detector_distance_m=a.focustodetectordistance,
                             detector_pixelsize_m=a.detector_pixelsize, magnification=mag,
                             voxelsize_m=voxelsize, n=a.n, nobj=nobj,
                             frame_ids_spec=a.frame_ids_spec, normalization_mode=mode)
    ...
    rec_args = SimpleNamespace(energy=a.energy, detector_pixelsize=a.detector_pixelsize,
        ...
        nz=a.n, n=a.n, nzobj=nobj, nobj=nobj, obj_dtype="complex64", rho=a.rho,
    ...
    rec.vars["proj"][:] = 0; rec.vars["prb"][:] = 1
    ...
    if rank == 0:
        probe = rec.vars["prb"].get(); projection = rec.vars["proj"].get()
    ...
    del rec; cp.get_default_memory_pool().free_all_blocks()
